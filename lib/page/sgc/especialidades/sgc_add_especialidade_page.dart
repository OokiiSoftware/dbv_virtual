...
class SgcAddEspecialidadePage extends StatefulWidget {
  const SgcAddEspecialidadePage({super.key});

  @override
  State<StatefulWidget> createState() => _State();
}
class _State extends State<SgcAddEspecialidadePage> with SingleTickerProviderStateMixin {

  final _log = const Log('AtribuirEspecialidadePage');

  final Map<String, Membro> _membros = {};
  final Map<String, Especialidade> _especialidades = {};
  ...
  Widget build(BuildContext context) {
    ...
    final espList = _especialidades.values.toList();

    return Scaffold(
      appBar: SgcAppBar(
        title: const Text('Atribuir Especialidades'),
        bottom: TabBar(
          ...
          tabs: [
            ...
            Tab(text: 'Especialidades (${_especialidades.length})',),
          ],
        ),
      ),
      body: Form(
                    ...
                    children: [
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          ...
                          child: const Text('Add Especialidade'),
                        ),
                      ),  // add
  ...
  void _onSaveTap() async {
    ...
    if (_especialidades.isEmpty) {
      Log.snack('Selecione uma ou mais especialidades', isError: true);
      return;
    }
    ...
    try {
      ...
      await SgcProvider.i.enviarEspecialidades(
          _membros.values.toList(), _especialidades.values.toList(), body);
    ...
    for (var e in res) {
      ...
      if (e.id == FirebaseProvider.i.user.id) {
        DialogBox(
          ...
          dismissible: false,
          content: const [
            Text('Você não pode atribuir especialidades a si mesmo.'),
          ],
        ).ok();
        continue;
      }
  ...
  void _selectEspecialidade() async {
    final item = await Popup(context).especialidade(_especialidades.keys.toList());
    if (item == null) return;

    _especialidades.clear();
    _especialidades.addAll(item);
    _setState();
  }
  ...
  void _onRemoveEspTap(Especialidade value) {
    _especialidades.remove(value.id);
    _setState();
  }
